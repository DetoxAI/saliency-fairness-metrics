...
class LRPHandler:
    ...
    def calculate(
        ...
        if hasattr(data_loader, "get_num_classes"):
            L = data_loader.get_num_classes()
        else:
            L = self.n_classes
            if L is None:
                raise ValueError(
                    """Number of classes not provided. Please provide the number of classes in LRPHandler constructor. 
                        You can also use the `WrappedDataLoader` class to get the number of classes automatically. 
                        Also you can just implement a `get_num_classes` method in your dataloader class."""
                )
